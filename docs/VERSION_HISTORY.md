...
Added a shared system log file for all monitor-level events (`save_system_logs`, `system_log_path`), serialized all terminal writes through a global mutex to prevent interleaved output from concurrent downloads, and switched download log files to append mode to prevent truncation.

Extended yt-dlp download success detection to track postprocessing and fatal error conditions from subprocess output (`postprocess_failed`, `fragment_failure`, `extractor_failed`, `auth_failure`, `disk_failure`, `process_crashed`), preventing false-success archiving when yt-dlp exits with errors. Cleaned up yt-dlp partial download residue (`.part-Frag`, `.part`, `.ytdl`, `.temp`) after successful `livestream_dl` fallback downloads.

Deferred Twitch archive writes to the next poll cycle when the stream is confirmed offline, matching the YouTube deferred-archive pattern. Throttled noisy yt-dlp `WARNING: [youtube] Video is no longer live` end-of-stream messages using `subprocess_progress_interval`.

Hardened the YouTube `/live` scraper to extract the video ID from the canonical URL, verify the channel ID against the page owner before trusting any live indicators, and anchor `"status":"LIVE"` checks to the specific video's JSON context. Deferred download manager startup until after the first full poll cycle to prevent stale or unverified status from triggering downloads immediately on launch.

Removed `--live-from-start` from the default Twitch twitch-dlp args since streammon's polling already catches streams near their start. Added `download_wait_retries` to the Twitch config, which detects when `--live-from-start` is stalling because the channel has no VOD data (repeated `Cannot find the playlist` lines with no download activity) and automatically restarts twitch-dlp without `--live-from-start` to capture from the live edge instead.
